...
class Storage:
    ...
    def write(self, f):
        ...
        group.create_dataset('stats', data=self.X2, chunks=(1, 7, 90))
...
class DataWriter:
    ...
    def store(self, contig, positions, examples, labels, stats):
        try:
            storage = self.storages[contig]
        except KeyError:
            storage = self.storages[contig] = Storage(contig, self.infer)

        storage.extend(positions, examples, labels, stats)
